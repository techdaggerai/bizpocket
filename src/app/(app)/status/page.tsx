  ...
  return (
    <div className="fixed inset-0 z-[90] flex flex-col bg-slate-800">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border)]">
        <button onClick={onClose} className="text-indigo-400 font-medium text-[14px]">Cancel</button>
        <p className="text-[15px] font-bold text-white">New Status</p>
        <button
          onClick={handlePublish}
          disabled={publishing || (type === 'text' ? !content.trim() : !imageUrl)}
          className="text-[14px] font-bold text-indigo-400 disabled:opacity-40"
        >
      ...
      <div className="flex border-b border-[var(--border)]">
        {(['text', 'image'] as const).map(t => (
          <button
            key={t}
            onClick={() => setType(t)}
            className={`flex-1 py-2.5 text-[13px] font-semibold capitalize transition-colors border-b-2 ${type === t ? 'border-[#4F46E5] text-indigo-400' : 'border-transparent text-[#9CA3AF]'}`}
          >
            {t === 'text' ? '✏️ Text' : '📷 Image'}
          </button>
        ))}
      ...
      <div className="border-t border-[var(--border)] p-4 space-y-3">
        ...
        {type === 'image' && imageUrl && (
          <input
            ...
            maxLength={100}
            className="w-full rounded-xl border border-slate-700 bg-slate-800 px-3 py-2.5 text-[14px] text-white placeholder-[#9CA3AF] focus:border-[#4F46E5] focus:outline-none"
          />
        )}
        {/* Change image button */}
        {type === 'image' && imageUrl && (
          <button
            ...
            className="text-[13px] text-indigo-400 font-medium"
          >
            Change photo
          </button>
        )}
        ...
        <div className="divide-y divide-slate-700">
          ...
          <div className="px-4 py-2">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[14px] font-semibold text-white">{myName}</p>
              ...
              <button
                ...
                className="h-9 w-9 rounded-full bg-[#EEF2FF] flex items-center justify-center text-indigo-400 hover:bg-[#4F46E5] hover:text-white transition-colors"
                ...
                {contactStatuses.map((group) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className={`text-[14px] truncate ${viewed ? 'font-medium text-[#374151]' : 'font-bold text-white'}`}>
                          {group.name}
                        </p>
          ...
          {contactStatuses.length === 0 && !loading && (
            <div className="flex flex-col items-center gap-3 py-16 px-6 text-center">
              ...
              <p className="text-[15px] font-semibold text-white">No updates yet</p>
