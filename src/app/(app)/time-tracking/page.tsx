  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between px-4">
        <div><h1 className="text-xl font-semibold text-white">Time Tracking</h1><p className="text-xs text-slate-400">{totalMinutes} min unbilled &middot; {formatCurrency(totalBillable, currency)}</p></div>
      ...
      <div className="rounded-xl border border-slate-700 bg-slate-800 p-4">
        <div className="flex items-center justify-between mb-3">
          <span className="text-xs font-medium text-slate-400 uppercase tracking-wider">Live Timer</span>
      ...
      : entries.length === 0 ? <div className="rounded-xl border border-dashed border-slate-700 p-8 text-center"><p className="text-sm text-slate-400">No time entries yet</p></div>
      : <div className="space-y-1.5">{entries.map(e => (
          <div key={e.id} className="flex items-center justify-between rounded-lg border border-slate-700 bg-slate-800 px-3 py-2.5">
            <div>
              ...
              <p className="text-[10px] text-slate-400">{e.date} &middot; {e.duration_minutes} min{e.is_invoiced ? ' &middot; Invoiced' : ''}</p>
            </div>
            <div className="text-right">
              <p className="text-sm font-mono font-medium">{formatCurrency(e.total_amount, currency)}</p>
              <span className={`text-[9px] ${e.is_billable ? 'text-[#16A34A]' : 'text-slate-400'}`}>{e.is_billable ? 'Billable' : 'Non-billable'}</span>
            </div>
          </div>
        ))}</div>}
    </div>
