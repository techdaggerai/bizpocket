...
export default function VoiceMessageBubble({ audioUrl, duration, originalText, translatedText, originalLanguage, targetLanguage, isOwn }: Props) {
  ...
  return (
    <div className={`max-w-[80%] px-3.5 py-2.5 ${isOwn ? 'rounded-[14px_14px_4px_14px] bg-[#4F46E5]' : 'rounded-[14px_14px_14px_4px] bg-[#f3f4f6]'}`}>
      <audio ref={audioRef} src={audioUrl} onEnded={() => setPlaying(false)} />
      <div className="flex items-center gap-2.5">
        <button onClick={togglePlay} className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${isOwn ? 'bg-white/20' : 'bg-[#e5e7eb]'}`}>
          {playing ? (
        ...
        <span className={`text-[11px] font-mono ${isOwn ? 'text-white/60' : 'text-[#9ca3af]'}`}>{fmt(duration)}</span>
      </div>

      {translatedText && (
        <div className={`mt-2 pt-2 ${isOwn ? 'border-t border-white/10' : 'border-t border-[#e5e7eb]'}`}>
          <div className="flex gap-1.5 mb-1.5">
            <button onClick={speakTranslation} className={`px-2.5 py-1 rounded-full text-[11px] font-medium flex items-center gap-1 ${playingTTS ? 'bg-[#F59E0B] text-white' : isOwn ? 'bg-white/15 text-white' : 'bg-[#fef3c7] text-[#92400e]'}`}>
            ...
            <button onClick={() => setShowText(!showText)} className={`px-2.5 py-1 rounded-full text-[11px] font-medium ${isOwn ? 'bg-white/15 text-white/80' : 'bg-[#f3f4f6] text-[#6b7280]'}`}>
              {showText ? 'Hide text' : 'Show text'}
            </button>
          </div>
          {showText && (
            <div className="text-xs leading-relaxed">
              {originalText && <p className={`mb-1 ${isOwn ? 'text-white/50' : 'text-[#9ca3af]'}`}>Original: {originalText}</p>}
