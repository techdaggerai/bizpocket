    ...
    <div className="bg-white w-full max-w-[210mm] mx-auto" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div className="px-8 py-6">
        <div className="flex justify-between mb-8">
          <div><p className="text-[10px] font-semibold uppercase tracking-wider text-[#999] mb-1">Bill To</p><p className="text-sm font-semibold text-[#0A0A0A]">{data.customer_name}</p>{data.customer_address && <p className="text-xs text-[#666] mt-0.5">{data.customer_address}</p>}{data.customer_phone && <p className="text-xs text-[#666]">{data.customer_phone}</p>}</div>
          <div className="text-right space-y-1"><div><span className="text-[10px] text-[#999]">Date: </span><span className="text-xs text-[#0A0A0A]">{data.date}</span></div>{data.due_date && <div><span className="text-[10px] text-[#999]">Due: </span><span className="text-xs text-[#0A0A0A]">{data.due_date}</span></div>}{data.company_tax_number && <div><span className="text-[10px] text-[#999]">Tax ID: </span><span className="text-xs text-[#0A0A0A]">{data.company_tax_number}</span></div>}</div>
        </div>
        <table className="w-full mb-6"><thead><tr className="bg-[#0A0A0A] text-white"><th className="text-left text-[10px] font-semibold px-3 py-2.5 uppercase tracking-wider">Description</th><th className="text-center text-[10px] font-semibold px-3 py-2.5 uppercase tracking-wider w-16">Qty</th><th className="text-right text-[10px] font-semibold px-3 py-2.5 uppercase tracking-wider w-28">Unit Price</th><th className="text-right text-[10px] font-semibold px-3 py-2.5 uppercase tracking-wider w-28">Total</th></tr></thead><tbody>{data.items.map((item, i) => (<tr key={i} className="border-b border-[#E5E5E5]"><td className="text-sm text-[#0A0A0A] px-3 py-3">{item.description}</td><td className="text-sm text-[#0A0A0A] text-center px-3 py-3">{item.quantity}</td><td className="text-sm text-[#0A0A0A] text-right px-3 py-3 font-mono">{fmt(item.unit_price, data.currency)}</td><td className="text-sm text-[#0A0A0A] text-right px-3 py-3 font-mono">{fmt(item.total, data.currency)}</td></tr>))}</tbody></table>
        <div className="flex justify-end mb-8"><div className="w-64"><div className="flex justify-between py-1.5 text-sm text-[#666]"><span>Subtotal</span><span className="font-mono">{fmt(data.subtotal, data.currency)}</span></div><div className="flex justify-between py-1.5 text-sm text-[#666]"><span>Tax ({(data.tax_rate * 100).toFixed(0)}%)</span><span className="font-mono">{fmt(data.tax_amount, data.currency)}</span></div><div className="flex justify-between py-2.5 border-t-2 border-[#0A0A0A] text-base font-bold text-[#0A0A0A]"><span>Total</span><span className="font-mono">{fmt(data.grand_total, data.currency)}</span></div></div></div>
        {data.bank_name && (<div className="border-t border-[#E5E5E5] pt-4"><p className="text-[10px] font-semibold uppercase tracking-wider text-[#999] mb-2">Payment Details</p><div className="text-xs text-[#666] space-y-0.5"><p>Bank: {data.bank_name} {data.bank_branch && `(${data.bank_branch})`}</p>{data.bank_account_name && <p>Account: {data.bank_account_name}</p>}{data.bank_account_number && <p>Number: {data.bank_account_number}</p>}</div></div>)}
        {data.notes && (<div className="mt-4 border-t border-[#E5E5E5] pt-4"><p className="text-[10px] font-semibold uppercase tracking-wider text-[#999] mb-1">Notes</p><p className="text-xs text-[#666]">{data.notes}</p></div>)}
...
function ModernTemplate({ data }: { data: InvoiceData }) {
  return (
    <div className="bg-white w-full max-w-[210mm] mx-auto" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div className="px-8 py-6">
        <div className="flex gap-4 mb-6 bg-[#F8F7FF] rounded-lg p-3"><div><span className="text-[10px] text-[#7C3AED] font-semibold">ISSUED</span><p className="text-sm text-[#0A0A0A] font-medium">{data.date}</p></div>{data.due_date && <div><span className="text-[10px] text-[#7C3AED] font-semibold">DUE</span><p className="text-sm text-[#0A0A0A] font-medium">{data.due_date}</p></div>}{data.status && <div className="ml-auto"><span className="text-[10px] text-[#7C3AED] font-semibold">STATUS</span><p className="text-sm text-[#0A0A0A] font-medium uppercase">{data.status}</p></div>}</div>
        <div className="mb-6"><p className="text-[10px] font-bold uppercase tracking-wider text-[#7C3AED] mb-1">Billed To</p><p className="text-sm font-semibold text-[#0A0A0A]">{data.customer_name}</p>{data.customer_address && <p className="text-xs text-[#666]">{data.customer_address}</p>}</div>
        <table className="w-full mb-6"><thead><tr className="bg-[#4F46E5] text-white"><th className="text-left text-[10px] font-semibold px-3 py-2.5 rounded-tl-lg">Item</th><th className="text-center text-[10px] font-semibold px-3 py-2.5 w-16">Qty</th><th className="text-right text-[10px] font-semibold px-3 py-2.5 w-28">Rate</th><th className="text-right text-[10px] font-semibold px-3 py-2.5 w-28 rounded-tr-lg">Amount</th></tr></thead><tbody>{data.items.map((item, i) => (<tr key={i} className={i % 2 === 0 ? 'bg-[#FAFAFE]' : 'bg-white'}><td className="text-sm text-[#0A0A0A] px-3 py-3">{item.description}</td><td className="text-sm text-center px-3 py-3">{item.quantity}</td><td className="text-sm text-right px-3 py-3 font-mono text-[#666]">{fmt(item.unit_price, data.currency)}</td><td className="text-sm text-right px-3 py-3 font-mono font-medium">{fmt(item.total, data.currency)}</td></tr>))}</tbody></table>
...
function JapaneseTemplate({ data }: { data: InvoiceData }) {
  return (
    <div className="bg-white w-full max-w-[210mm] mx-auto" style={{ fontFamily: "'DM Sans', 'Noto Sans JP', sans-serif" }}>
      ...
      <div className="px-8 py-6">
        <div className="text-center mb-6"><h1 className="text-3xl font-bold tracking-[0.3em] text-[#0A0A0A]">請求書</h1><p className="text-[10px] text-[#999] mt-1 tracking-widest">INVOICE</p></div>
        <div className="flex justify-between mb-6"><div><p className="text-sm font-semibold text-[#0A0A0A]">{data.customer_name} <span className="text-[#999]">様</span></p>{data.customer_address && <p className="text-xs text-[#666] mt-0.5">{data.customer_address}</p>}</div><div className="text-right flex items-start gap-4"><div><p className="text-sm font-semibold text-[#0A0A0A]">{data.company_name}</p>{data.company_address && <p className="text-xs text-[#666] mt-0.5">{data.company_address}</p>}{data.company_phone && <p className="text-xs text-[#666]">TEL: {data.company_phone}</p>}{data.t_number && <p className="text-xs text-[#666]">登録番号: {data.t_number}</p>}</div><div className="w-14 h-14 rounded-full border-2 border-[#DC2626] flex items-center justify-center shrink-0"><span className="text-[#DC2626] text-xs font-bold" style={{ writingMode: 'vertical-rl' }}>{data.company_name.slice(0, 3)}</span></div></div></div>
        <div className="flex justify-between mb-4 text-xs"><div><span className="text-[#999]">請求書番号: </span><span className="text-[#0A0A0A]">{data.invoice_number}</span></div><div><span className="text-[#999]">請求日: </span><span className="text-[#0A0A0A]">{data.date}</span></div>{data.due_date && <div><span className="text-[#999]">支払期限: </span><span className="text-[#0A0A0A]">{data.due_date}</span></div>}</div>
        <div className="bg-[#FAFAFA] border border-[#E5E5E5] rounded p-3 mb-6 flex justify-between items-center"><span className="text-sm font-semibold text-[#0A0A0A]">ご請求金額</span><span className="text-xl font-bold text-[#0A0A0A] font-mono">{fmt(data.grand_total, data.currency)}</span></div>
        ...
        {data.bank_name && (<div className="border border-[#E5E5E5] rounded p-3 mb-4"><p className="text-[10px] font-semibold text-[#999] mb-1">お振込先</p><div className="text-xs text-[#0A0A0A] space-y-0.5"><p>{data.bank_name} {data.bank_branch}</p>{data.bank_account_name && <p>口座名義: {data.bank_account_name}</p>}{data.bank_account_number && <p>口座番号: {data.bank_account_number}</p>}</div></div>)}
  ...
  return (
    <div className="bg-white w-full max-w-[210mm] mx-auto" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div className="px-6 py-4">
        <div className="flex justify-between items-start mb-4"><div><p className="text-sm font-bold text-[#0A0A0A]">{data.company_name}</p>{data.company_address && <p className="text-[10px] text-[#999]">{data.company_address}</p>}</div><div className="text-right"><p className="text-xs font-bold text-[#16A34A] uppercase tracking-wider">Invoice</p><p className="text-[10px] text-[#999]">{data.invoice_number} · {data.date}</p></div></div>
        <div className="flex gap-8 mb-4 text-xs"><div><span className="text-[10px] text-[#16A34A] font-semibold uppercase">From</span><p className="text-[#0A0A0A] font-medium">{data.company_name}</p>{data.company_phone && <p className="text-[#999]">{data.company_phone}</p>}</div><div><span className="text-[10px] text-[#16A34A] font-semibold uppercase">To</span><p className="text-[#0A0A0A] font-medium">{data.customer_name}</p>{data.customer_phone && <p className="text-[#999]">{data.customer_phone}</p>}</div></div>
        <div className="border-t border-b border-[#E5E5E5] mb-4"><div className="flex text-[9px] font-semibold uppercase tracking-wider text-[#999] py-1.5 px-2"><span className="flex-1">Description</span><span className="w-12 text-center">Qty</span><span className="w-20 text-right">Price</span><span className="w-20 text-right">Total</span></div>{data.items.map((item, i) => (<div key={i} className="flex text-xs py-2 px-2 border-t border-[#F5F5F5]"><span className="flex-1 text-[#0A0A0A]">{item.description}</span><span className="w-12 text-center text-[#666]">{item.quantity}</span><span className="w-20 text-right font-mono text-[#666]">{fmt(item.unit_price, data.currency)}</span><span className="w-20 text-right font-mono font-medium text-[#0A0A0A]">{fmt(item.total, data.currency)}</span></div>))}</div>
...
function BoldTemplate({ data }: { data: InvoiceData }) {
  return (
    <div className="bg-[#0A0A0A] text-white w-full max-w-[210mm] mx-auto" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <div className="px-8 py-6">
        <div className="flex justify-between items-center mb-8"><h1 className="text-2xl font-black text-white">{data.company_name}</h1><div className="bg-[#FACC15] text-[#0A0A0A] px-4 py-1.5"><p className="text-sm font-black uppercase tracking-wider">Invoice</p></div></div>
...
function ReceiptTemplate({ data }: { data: InvoiceData }) {
  return (
    <div className="bg-white w-full max-w-[210mm] mx-auto flex justify-center py-4"><div className="w-[80mm] border border-dashed border-[#CCC] p-4" style={{ fontFamily: "'Courier New', monospace" }}>
      <div className="text-center mb-4 pb-3 border-b border-dashed border-[#CCC]"><p className="text-base font-bold text-[#0A0A0A]">{data.company_name}</p>{data.company_address && <p className="text-[10px] text-[#666]">{data.company_address}</p>}{data.company_phone && <p className="text-[10px] text-[#666]">{data.company_phone}</p>}{data.t_number && <p className="text-[10px] text-[#666]">登録番号: {data.t_number}</p>}</div>
      <div className="mb-3 text-[10px] text-[#666]"><p>No: {data.invoice_number}</p><p>Date: {data.date}</p><p>To: {data.customer_name}</p></div>
      <div className="border-t border-dashed border-[#CCC] pt-2 mb-2">{data.items.map((item, i) => (<div key={i} className="flex justify-between py-1 text-xs text-[#0A0A0A]"><span className="flex-1">{item.description} x{item.quantity}</span><span className="font-mono">{fmt(item.total, data.currency)}</span></div>))}</div>
      <div className="border-t border-dashed border-[#CCC] pt-2 space-y-1"><div className="flex justify-between text-xs text-[#666]"><span>Subtotal</span><span className="font-mono">{fmt(data.subtotal, data.currency)}</span></div><div className="flex justify-between text-xs text-[#666]"><span>Tax</span><span className="font-mono">{fmt(data.tax_amount, data.currency)}</span></div><div className="flex justify-between text-sm font-bold text-[#0A0A0A] border-t border-dashed border-[#CCC] pt-1"><span>TOTAL</span><span className="font-mono">{fmt(data.grand_total, data.currency)}</span></div></div>
...
function CorporateTemplate({ data }: { data: InvoiceData }) {
  return (
    <div className="bg-white w-full max-w-[210mm] mx-auto" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="px-8 py-6">
        <div className="grid grid-cols-2 gap-4 mb-8 bg-[#F0F4F8] rounded-lg p-4"><div><p className="text-[10px] font-semibold uppercase text-[#1E3A5F] mb-1">Bill To</p><p className="text-sm font-medium text-[#0A0A0A]">{data.customer_name}</p>{data.customer_address && <p className="text-xs text-[#666]">{data.customer_address}</p>}</div><div className="text-right">{data.due_date && <p className="text-xs text-[#666]">Due: {data.due_date}</p>}</div></div>
        ...
        {data.notes && (<div className="mt-4 border-t border-[#E5E5E5] pt-4"><p className="text-[10px] font-semibold uppercase text-[#1E3A5F] mb-1">Notes</p><p className="text-xs text-[#666]">{data.notes}</p></div>)}
  ...
  return (
    <div className="bg-white w-full max-w-[210mm] mx-auto" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <div className="px-10 py-10">
        <div className="flex justify-between items-start mb-16"><div><p className="text-[10px] uppercase tracking-[0.2em] text-[#BBB]">From</p><h1 className="text-lg font-medium text-[#0A0A0A] mt-1">{data.company_name}</h1></div><div className="text-right"><p className="text-[10px] uppercase tracking-[0.2em] text-[#BBB]">Invoice</p><p className="text-sm text-[#0A0A0A] mt-1">{data.invoice_number}</p><p className="text-xs text-[#999]">{data.date}</p></div></div>
        <div className="mb-16"><p className="text-[10px] uppercase tracking-[0.2em] text-[#BBB]">To</p><p className="text-sm text-[#0A0A0A] mt-1">{data.customer_name}</p>{data.customer_address && <p className="text-xs text-[#999]">{data.customer_address}</p>}</div>
        <div className="mb-16">{data.items.map((item, i) => (<div key={i} className="flex justify-between py-4 border-b border-[#F0F0F0]"><div className="flex-1"><p className="text-sm text-[#0A0A0A]">{item.description}</p><p className="text-[10px] text-[#999] mt-0.5">{item.quantity} x {fmt(item.unit_price, data.currency)}</p></div><p className="text-sm font-mono text-[#0A0A0A]">{fmt(item.total, data.currency)}</p></div>))}</div>
        <div className="flex justify-end mb-16"><div className="w-48 space-y-2"><div className="flex justify-between text-xs text-[#999]"><span>Subtotal</span><span className="font-mono">{fmt(data.subtotal, data.currency)}</span></div><div className="flex justify-between text-xs text-[#999]"><span>Tax</span><span className="font-mono">{fmt(data.tax_amount, data.currency)}</span></div><div className="flex justify-between text-lg font-medium text-[#0A0A0A] pt-2 border-t border-[#0A0A0A]"><span>Total</span><span className="font-mono">{fmt(data.grand_total, data.currency)}</span></div></div></div>
