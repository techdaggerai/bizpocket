...
export default function PublicPocketChat({ conversationId, ownerId, ownerName, ownerLanguage, orgId, publicToken, context = 'order' }: PublicPocketChatProps) {
  ...
  if (!chatOpen) {
    return (
      <div className="fixed bottom-5 right-5 z-50">
        ...
        <div className="absolute bottom-16 right-0 bg-slate-800 rounded-xl px-3.5 py-2.5 shadow-lg whitespace-nowrap text-[13px] font-medium text-[#1f2937]">
          Questions? Chat with us
          <div className="absolute -bottom-1.5 right-5 w-3 h-3 bg-slate-800 transform rotate-45 shadow-sm" />
        </div>
      </div>
    );
  }
  ...
  if (!isSetup) {
    return (
      <div className="fixed bottom-5 right-5 w-[360px] max-w-[calc(100vw-32px)] bg-slate-800 rounded-2xl shadow-2xl overflow-hidden z-50">
        ...
        <div className="p-5 space-y-3">
          ...
          <select value={customerLanguage} onChange={(e) => setCustomerLanguage(e.target.value)}
            className="w-full px-3.5 py-2.5 rounded-[10px] border border-slate-700 text-sm outline-none focus:border-[#4F46E5] bg-slate-800">
  ...
  return (
    <div className="fixed bottom-5 right-5 w-[380px] max-w-[calc(100vw-32px)] h-[520px] max-h-[calc(100vh-40px)] bg-slate-800 rounded-2xl shadow-2xl overflow-hidden z-50 flex flex-col">
      <div className="bg-[#4F46E5] px-4 py-3.5 flex justify-between items-center shrink-0">
        <div className="flex items-center gap-2.5">
          <div className="w-[34px] h-[34px] rounded-[10px] bg-slate-800/20 flex items-center justify-center text-white text-sm font-semibold">{ownerName.charAt(0).toUpperCase()}</div>
          <div>
          ...
          return (
            <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
              <div onClick={() => { if (display.isTranslated) setShowOriginal(showOriginal === msg.id ? null : msg.id); }}
                className={`max-w-[80%] px-3.5 py-2.5 ${isMe ? 'rounded-[14px_14px_4px_14px] bg-[#4F46E5]' : 'rounded-[14px_14px_14px_4px] bg-slate-800'} ${display.isTranslated ? 'cursor-pointer' : ''}`}>
      ...
      <div className="border-t border-[#f3f4f6] px-3 py-2.5 flex items-center gap-2 shrink-0">
        <div className="px-2 py-1 rounded-md bg-slate-800 text-xs text-slate-400 shrink-0">{LANGUAGES[customerLanguage]?.flag}</div>
        <input type="text" placeholder="Type a message..." value={input} onChange={(e) => setInput(e.target.value)}
          onKeyDown={(e) => e.key === 'Enter' && handleSend()}
          className="flex-1 px-3 py-2.5 rounded-[10px] border border-slate-700 text-[13px] outline-none" />
