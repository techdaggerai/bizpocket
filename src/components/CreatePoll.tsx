    ...
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center">
      ...
      <div className="relative w-full max-w-md sm:mx-4 bg-slate-800 rounded-t-2xl sm:rounded-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-5 space-y-4">
          {/* Question */}
          <div>
            <label className="text-sm font-medium text-slate-300 block mb-1.5">Question</label>
            <input
              type="text"
              value={question}
          ...
          {/* Options */}
          <div>
            <label className="text-sm font-medium text-slate-300 block mb-1.5">Options</label>
          ...
          {/* Settings */}
          <div className="space-y-3 pt-2 border-t border-[var(--border)] border-slate-700">
            <label className="flex items-center justify-between">
              <span className="text-sm text-slate-300">Allow multiple answers</span>
              <button
                onClick={() => setAllowMultiple(!allowMultiple)}
                className="relative w-10 h-[22px] rounded-full transition-colors"
                style={{ backgroundColor: allowMultiple ? '#4F46E5' : '#475569' }}
              >
                <span className="absolute top-[2px] h-[18px] w-[18px] rounded-full bg-slate-800 shadow transition-transform" style={{ left: allowMultiple ? 20 : 2 }} />
              </button>
            </label>
            <label className="flex items-center justify-between">
              <span className="text-sm text-slate-300">Anonymous votes</span>
              <button
                onClick={() => setAnonymous(!anonymous)}
                className="relative w-10 h-[22px] rounded-full transition-colors"
                style={{ backgroundColor: anonymous ? '#4F46E5' : '#475569' }}
              >
                <span className="absolute top-[2px] h-[18px] w-[18px] rounded-full bg-slate-800 shadow transition-transform" style={{ left: anonymous ? 20 : 2 }} />
              </button>
            </label>
          </div>
        </div>
