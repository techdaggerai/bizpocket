...
export default function MessageActionSheet({
  ...
  return (
    <div className="fixed inset-0 z-[55]">
      ...
      {/* Sheet */}
      <div className="absolute bottom-0 left-0 right-0 bg-slate-800 rounded-t-2xl animate-slide-up safe-bottom overflow-hidden">
        {/* Drag handle */}
        ...
        {/* Message preview */}
        <div className="px-4 pb-3 border-b border-[var(--border)] border-slate-700">
          <p className="text-[11px] font-medium text-[#9CA3AF] mb-0.5">{senderName}</p>
          <p className="text-[13px] text-slate-300 line-clamp-2 leading-relaxed">{text}</p>
        </div>
        ...
        {/* Cancel */}
        <div className="border-t border-[var(--border)] border-slate-700">
          <button onClick={onClose} className="w-full py-3.5 text-[15px] font-medium text-[#9CA3AF] active:bg-slate-700 active:bg-slate-700">
            Cancel
          </button>
        </div>
      </div>
