...
export default function PocketChatTypingIndicator({ contactName, compact = false }: Props) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setFade(false);
      setTimeout(() => { setLangIndex(i => (i + 1) % languages.length); setFade(true); }, 150);
    }, 500);
  ...
  return (
    <div className={`flex items-center ${compact ? 'gap-2 py-1.5 px-1' : 'gap-3 py-2'}`}>
      <svg width={s} height={s} viewBox="0 0 88 88" fill="none" style={{ animation: 'pcBreathe 1.8s ease-in-out infinite' }}>
        ...
        <text x="55.5" y="72" fontSize="9.5" fontWeight="700" fill="white" textAnchor="middle" fontFamily="sans-serif" style={{ opacity: fade ? 1 : 0, transition: 'opacity 0.15s ease' }}>{lang.text}</text>
      </svg>
          ...
          <div className="flex gap-[3px] items-center">
            {[0, 1, 2].map(d => (
              <div key={d} className={`${compact ? 'w-[5px] h-[5px]' : 'w-1.5 h-1.5'} rounded-full`}
                style={{ background: d === 1 ? '#F59E0B' : '#4F46E5', animation: 'pcWave 1.2s ease-in-out infinite', animationDelay: `${d * 0.15}s` }} />
            ))}
          </div>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-[11px] text-[#F59E0B] font-medium min-w-[70px]" style={{ opacity: fade ? 1 : 0, transition: 'opacity 0.15s ease' }}>{lang.label}</span>
          <div className="w-[100px] h-[2px] rounded-sm bg-[#f3f4f6] overflow-hidden">
            <div className="w-[30px] h-[2px] rounded-sm bg-[#F59E0B]" style={{ animation: 'pcProgress 1.5s ease-in-out infinite' }} />
          </div>
        </div>
      </div>
    </div>
